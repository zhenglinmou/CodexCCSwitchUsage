...
function hostMatches(processInfo, hostPath, port, allInstances) {
  if (!processInfo?.commandLine || !String(processInfo.commandLine).includes(hostPath)) return false;
  if (!allInstances) return true;
  return new RegExp(`(?:^|\\s)--port(?:=|\\s+)${port}(?=\\s|$)`).test(processInfo.commandLine);
}
...
export async function stopHost({
  ...
  for (const processInfo of processes) {
    if (hostMatches(processInfo, hostPath, port, allInstances)) candidates.add(processInfo.pid);
  }
  let stopped = 0;
