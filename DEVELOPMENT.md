...
The injected refresh and Hub buttons append a bounded invisible action marker (`refresh` or `open-hub`) to `document.title`. The host reads the short-lived `/json/list` HTTP snapshot every 1,000 ms, consumes a new marker, and restores the visible title during the one-shot result update. This title channel carries no provider data or credential and avoids a persistent CDP WebSocket. Each CDP HTTP request uses `Connection: close`; full injector audits remain limited to 300,000 ms.
