...
- 不再保持浏览器级或页面级 CDP WebSocket：每秒只读取一次短生命周期 `/json/list` 标题快照来接收刷新/Hub 点击，只有启动、结果推送和五分钟安全审计才在唯一主页面上瞬时注入并立即断开；内置 Browse/WebView 存在时暂停全部注入。
...
- AnyRouter、AgentRouter 等网页登录供应商始终显示独立的“官网登录/重新登录官网”入口；入口不依赖 WAF 错误能否被准确分类，也不会触发额度查询。
