...
test('page-action acknowledgement is isolated from payload updates and preserves a newer title', async () => {
  ...
  assert.equal(client.closed, true);
  assert.equal(client.expressions.length, 2);
  assert.match(client.expressions[1], /document\.title !==/);
  assert.doesNotMatch(client.expressions[1], /\?\.update|\?\.mount|FULL_INJECTOR/);
});
