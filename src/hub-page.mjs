...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}
    ...
    .binding-current{cursor:default;color:var(--green);background:rgba(86,213,139,.08)}
    .binding-current:hover{border-color:var(--line);background:rgba(86,213,139,.08);transform:none}
    ...
    .method-dialog{width:min(760px,calc(100% - 24px));max-height:min(820px,calc(100vh - 32px));padding:0;border:1px solid var(--line);border-radius:18px;background:var(--panel);color:var(--text);box-shadow:0 30px 90px rgba(0,0,0,.58);overflow:hidden}
    ...
    .template-config{margin:0 0 16px;padding:0 0 16px;border-bottom:1px solid var(--line)}
    ...
    .template-select{width:100%;height:38px;padding:0 34px 0 10px;border:1px solid var(--line);border-radius:8px;background:var(--panel-2);color:var(--text);font:inherit;font-size:13px}
    .template-description{display:block;min-height:34px;margin-top:7px;color:var(--muted);font-size:11px;line-height:1.5}
    .template-actions{display:flex;justify-content:flex-end;gap:8px;flex-wrap:wrap;margin-top:12px}
    .template-result{margin-top:12px;padding:10px 12px;border-left:3px solid var(--blue);background:var(--panel-2);font-size:12px}
    ...
    .probe-line.needs-action{color:var(--orange)}
    ...
    @media(max-width:760px){.shell{width:min(100% - 20px,1180px);padding-top:24px}.topbar{display:block}.actions{justify-content:flex-start;margin-top:18px}.overview{grid-template-columns:repeat(2,1fr)}.grid{grid-template-columns:1fr}.toolbar{align-items:flex-end}.sync{display:none}.card-foot{flex-wrap:wrap}.source{width:100%}.card-actions{width:100%;justify-content:flex-end}.method-fields,.template-grid{grid-template-columns:1fr}.method-row.wide{grid-column:auto}.method-head,.method-body{padding-left:16px;padding-right:16px}}
...
<body>
  ...
  <dialog id="method-dialog" class="method-dialog" aria-labelledby="method-title">
    <div class="method-panel">
      <header class="method-head"><div><span class="method-kicker">Provider templates</span><h2 id="method-title" class="method-title">模板与查询方式</h2><p id="method-subtitle" class="method-subtitle"></p></div><button id="method-close" class="method-close" type="button" aria-label="关闭">×</button></header>
      <div class="method-body"><section class="template-config" aria-label="供应商模板"><div class="template-grid"><label class="template-field"><span class="template-label">余额额度模板</span><select id="balance-template" class="template-select"></select><span id="balance-template-description" class="template-description"></span></label><label class="template-field"><span class="template-label">逐请求 Token / 扣费模板</span><select id="request-template" class="template-select"></select><span id="request-template-description" class="template-description"></span></label></div><div class="template-actions"><button id="template-reset" class="button small" type="button">恢复内置</button><button id="template-detect" class="button small" type="button">自动识别</button><button id="template-test" class="button small" type="button">测试所选</button><button id="template-save" class="button small primary" type="button">保存模板</button></div><div id="template-result" class="template-result" role="status" hidden></div></section><dl id="method-fields" class="method-fields"></dl><section id="method-notes" class="method-notes"><h3>实现说明</h3><ul id="method-note-list"></ul></section><p class="method-warning">这里只显示请求结构，不显示 API Key、Cookie、Bearer Token、账号文件内容或其他认证原文。</p></div>
    </div>
  </dialog>
  ...
  <script nonce="${nonce}">
    ...
    function stateRenderKey(next){const companion=next.companion||{connected:false,clients:[]};const clients=(companion.clients||[]).map(client=>[client.ref,client.browser,client.version,[...(client.sessions||[])].sort()]);return JSON.stringify([next.revision,next.refreshing,next.lastFullRefreshAt,companion.connected,companion.compatibilityError||'',clients])}
    ...
    function companionTargets(){const clients=(state.companion?.clients||[]).filter(client=>client?.ref&&client?.browser).slice().sort((left,right)=>String(left.browser).localeCompare(String(right.browser)));const totals={};for(const client of clients)totals[client.browser]=(totals[client.browser]||0)+1;const seen={};return clients.map(client=>{seen[client.browser]=(seen[client.browser]||0)+1;return{ref:client.ref,browser:client.browser,label:client.browser+(totals[client.browser]>1?' '+seen[client.browser]:''),sessions:Array.isArray(client.sessions)?client.sessions:[]}})}
    ...
    async function refreshOne(id){try{await post('refresh',{providerId:id});showToast('已开始重新查询');scheduleOperationMonitor()}catch(error){showToast(error.message)}await load()}
    async function login(id,clientRef='',browser=''){try{const result=await post('login',{providerId:id,clientRef});showToast(result.provider?.message||(result.provider?.status==='ok'?'同步成功':browser?'已发送到 '+browser:'操作已完成'))}catch(error){showToast(error.message)}await load()}
    async function bindAccount(id,clientRef,browser){try{const result=await post('account-binding',{providerId:id,clientRef});showToast(result.provider?.bindingAttemptFailed?result.provider.message:'已绑定 '+browser+' AnyRouter 账号并完成刷新')}catch(error){showToast(error.message)}await load()}
    ...
    function renderCard(item){const card=el('article','card'+(item.current?' current':''));const head=el('div','card-head');const titleWrap=el('div');titleWrap.append(el('h2','provider-name',item.name));const badges=el('div','badges');const [label,tone]=statusMeta(item);badges.append(el('span','badge '+tone,label));if(item.current)badges.append(el('span','badge current-label','CCSwitch 当前'));titleWrap.append(badges);head.append(titleWrap);card.append(head);
      ...
      const foot=el('div','card-foot');const balanceUpdatedAt=usage?.updatedAt||item.lastSuccessAt||item.updatedAt;foot.append(el('span','source',sourceLabel(item)+' · '+age(balanceUpdatedAt)));const actions=el('div','card-actions');actions.append(action('模板',()=>showQueryMethod(item)));const loginInfo=loginLinkFor(item);const targets=companionTargets();const companionConnected=targets.length>0;if(item.accountBindingSupported&&companionConnected){const binding=item.accountBinding;let boundTargetShown=false;for(const target of targets){const hasSession=target.sessions.includes('https://anyrouter.top');const selected=binding?.clientRef===target.ref;if(selected){boundTargetShown=true;actions.append(el('span','button small binding-current','已绑定 '+target.label));if(!hasSession)actions.append(action('在 '+target.label+' 验证',()=>login(item.id,target.ref,target.browser),true));continue}const bindLabel=binding?'改绑 '+target.label:'绑定 '+target.label;if(hasSession)actions.append(action(bindLabel,()=>bindAccount(item.id,target.ref,target.browser),!binding));else actions.append(action('在 '+target.label+' 验证',()=>login(item.id,target.ref,target.browser),false))}if(binding&&!boundTargetShown)actions.append(el('span','button small binding-current','已绑定 '+binding.browser+'（离线）'));if(binding)actions.append(action('解除绑定',()=>clearAccountBinding(item.id)))}else if(loginInfo){if(companionConnected){const verb=loginInfo.primary?'验证':'登录';for(const target of targets)actions.append(action('在 '+target.label+' '+verb,()=>login(item.id,target.ref,target.browser),loginInfo.primary))}else{const loginLink=el('a','button small'+(loginInfo.primary?' primary':''),loginInfo.label);loginLink.href=loginInfo.href;loginLink.target='_blank';loginLink.rel='noreferrer';actions.append(loginLink)}}else if(item.websiteUrl){const link=el('a','button small','官网');link.href=item.websiteUrl;link.target='_blank';link.rel='noreferrer';actions.append(link)}if(item.sessionSyncRequired&&!companionConnected)actions.append(action('同步现有会话',()=>login(item.id),true));actions.append(action(item.refreshing?'查询中':'刷新',()=>refreshOne(item.id)));foot.append(actions);card.append(foot);return card}
    function render(){lastRenderedAt=Date.now();const providers=state.providers||[];document.getElementById('count-all').textContent=providers.length;document.getElementById('count-ok').textContent=providers.filter(item=>item.status==='ok').length;document.getElementById('count-attention').textContent=providers.filter(item=>['error','login-required','degraded'].includes(item.status)).length;document.getElementById('count-loading').textContent=providers.filter(item=>item.refreshing||item.status==='loading').length;refreshAll.disabled=Boolean(state.refreshing);refreshAll.textContent=state.refreshing?'正在刷新…':'刷新全部';const companion=state.companion||{connected:false,clients:[]};copyCompanion.textContent=companion.connected?'浏览器伴侣已连接':'连接现有浏览器';const summary=companionSummary(companion.clients);const browserText=summary?' · '+summary:companion.compatibilityError?' · '+companion.compatibilityError:' · 浏览器伴侣未连接';sync.textContent=(state.refreshing?'正在查询供应商':'本机数据已同步')+browserText+(state.lastFullRefreshAt?' · '+age(state.lastFullRefreshAt):'');let shown=providers;if(filter==='ok')shown=shown.filter(item=>item.status==='ok');if(filter==='attention')shown=shown.filter(item=>['error','login-required','degraded'].includes(item.status));grid.replaceChildren(...(shown.length?shown.map(renderCard):[el('div','empty','当前筛选下没有供应商')]))}
    ...
    document.addEventListener('visibilitychange',()=>{if(document.visibilityState==='hidden'){clearTimeout(companionMonitorTimer);return}load().then(()=>{if(state.refreshing)scheduleOperationMonitor();scheduleCompanionMonitor()})});
    load().then(()=>scheduleCompanionMonitor());
  </script>
</body>
</html>`;
}
