...
<body>
  <main class="shell">
    ...
    <aside class="privacy"><span aria-hidden="true">🔒</span><div><strong>Cookie 不会离开你的日常浏览器。</strong> API Key 与 OpenAI Token 只在本机宿主内存中用于请求；WAF 站点由现有 Edge/Chrome 伴侣在同源页面内查询。Hub 页面只接收归一化后的余额和状态，CCSwitch 数据库始终只读。</div></aside>
  </main>
  ...
  <script nonce="${nonce}">
    ...
    async function login(id){try{await post('login',{providerId:id});showToast('已在现有浏览器中打开登录页')}catch(error){showToast(error.message)}await load()}
    ...
    copyCompanion.addEventListener('click',async()=>{const token=API.split('/').filter(Boolean).at(-1)||'';try{await navigator.clipboard.writeText(token);showToast('浏览器伴侣连接码已复制；请粘贴到伴侣扩展弹窗')}catch{showToast('复制失败，请从本机 runtime/hub-token 读取连接码')}});
