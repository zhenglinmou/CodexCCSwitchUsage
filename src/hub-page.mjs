...
<body>
  <main class="shell">
    <header class="topbar">
      <div class="brand"><span class="brand-mark">CCSwitch</span><h1>Balance Hub</h1><span id="app-version" class="version">v3</span></div>
      <div class="actions">
  ...
  <script nonce="${nonce}">
    ...
    let operationMonitorGeneration=0;
    ...
    function refreshRelativeAges(){lastRenderedAt=Date.now();const providers=new Map((state.providers||[]).map(item=>[String(item.id),item]));for(const row of grid.querySelectorAll('[data-provider-id]')){const item=providers.get(String(row.dataset.providerId||''));if(!item)continue;const balanceUpdatedAt=item.usage?.updatedAt||item.lastSuccessAt||item.updatedAt;const sourceAge=row.querySelector('.cell-source .cell-secondary');if(sourceAge)sourceAge.textContent=age(balanceUpdatedAt);const attemptAge=row.querySelector('.cell-latency .cell-secondary');if(attemptAge)attemptAge.textContent=item.lastAttemptAt?'最近尝试 '+age(item.lastAttemptAt):''}const browserText=companionSummary(state.companion?.clients);sync.textContent=(state.refreshing?'正在查询使用中供应商':'本机数据已同步')+(browserText?' · '+browserText:'')+(state.lastFullRefreshAt?' · 全量刷新 '+age(state.lastFullRefreshAt):'')}
    function scheduleSearchRender(){if(searchRenderFrame)return;searchRenderFrame=requestAnimationFrame(()=>{searchRenderFrame=0;render()})}
    ...
    function stateRenderKey(next){const companion=next.companion||{connected:false,clients:[]};const clients=(companion.clients||[]).map(client=>[client.ref,client.browser,client.version,[...(client.sessions||[])].sort()]);return JSON.stringify([next.revision,next.refreshing,next.lastFullRefreshAt,next.cacheError||'',companion.connected,companion.compatibilityError||'',clients,next.preferences||{},next.diagnostics||{},next.browserOrigins||[]])}
    ...
    async function copyCompanionToken(){try{const response=await fetch(API+'/companion/pairing',{cache:'no-store'});const payload=await response.json().catch(()=>({}));if(!response.ok||!/^[A-Za-z0-9_-]{32,128}$/.test(String(payload.companionToken||'')))throw new Error('连接码不可用');await navigator.clipboard.writeText(payload.companionToken);showToast(state.companion?.connected?'浏览器已连接，无需重复配置；连接码也已复制':'浏览器伴侣连接码已复制')}catch{showToast(state.companion?.connected?'浏览器伴侣当前已连接，无需重复配置':'复制失败，请从本机 runtime/companion-token 读取连接码')}}
    ...
    function diagnosticCheckItems(){const allProviders=state.providers||[];const providers=activeProviders(allProviders);const ignoredCount=allProviders.length-providers.length;const d=state.diagnostics||{};const companion=state.companion||{connected:false,clients:[]};const browserCount=providers.filter(providerNeedsBrowser).length;const attentionCount=providers.filter(providerNeedsAttention).length;const versions=(companion.clients||[]).map(client=>String(client.version||'')).filter(Boolean);const expected=String(d.expectedCompanionVersion||'');const versionOk=!versions.length||!expected||versions.every(value=>value===expected);return[
      ...
      {tone:state.cacheError?'error':'ok',title:'Hub 状态持久化',detail:state.cacheError||'缓存写入正常'},
    ...
    async function load(){try{const response=await fetch(API+'/state',{cache:'no-store'});if(!response.ok)throw new Error('Hub 服务不可用');const next=await response.json();const nextRenderKey=stateRenderKey(next);const authChanged=reconcileAuthChecks(next);const ageRefreshDue=Date.now()-lastRenderedAt>=60000;const shouldRender=authChanged||nextRenderKey!==renderKey;state=next;if(shouldRender){renderKey=nextRenderKey;render()}else if(ageRefreshDue)refreshRelativeAges()}catch(error){sync.textContent=error.message}}
    async function loadCompanionStatus(){try{const response=await fetch(API+'/companion/status',{cache:'no-store'});if(!response.ok)return;const payload=await response.json();const next={...state,companion:payload.companion||{connected:false,clients:[]},diagnostics:payload.diagnostics||state.diagnostics};const nextRenderKey=stateRenderKey(next);const ageRefreshDue=Date.now()-lastRenderedAt>=60000;const shouldRender=nextRenderKey!==renderKey;state=next;if(shouldRender){renderKey=nextRenderKey;render()}else if(ageRefreshDue)refreshRelativeAges()}catch{}}
    function cancelOperationMonitor(){operationMonitorGeneration+=1;clearTimeout(operationMonitorTimer);operationMonitorTimer=0}
    function scheduleOperationMonitor(){cancelOperationMonitor();operationMonitorDeadline=Date.now()+600000;const generation=operationMonitorGeneration;const monitor=async()=>{if(generation!==operationMonitorGeneration||document.visibilityState==='hidden'||Date.now()>=operationMonitorDeadline)return;await load();if(generation!==operationMonitorGeneration||document.visibilityState==='hidden'||Date.now()>=operationMonitorDeadline)return;const checking=[...authChecks.values()].some(entry=>entry.phase==='checking');if(state.refreshing||checking)operationMonitorTimer=setTimeout(monitor,750)};operationMonitorTimer=setTimeout(monitor,100)}
    ...
    searchInput.addEventListener('input',()=>{query=searchInput.value;scheduleSearchRender()});
    ...
    document.addEventListener('visibilitychange',()=>{if(document.visibilityState==='hidden'){markHubLeft();cancelOperationMonitor();clearTimeout(companionMonitorTimer);return}load().then(()=>{if(state.refreshing||[...authChecks.values()].some(entry=>entry.phase==='checking'))scheduleOperationMonitor();scheduleCompanionMonitor();maybeAutoCheckPending()})});
