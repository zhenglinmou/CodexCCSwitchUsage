...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}
    ...
    .provider-list.cards .provider-main{grid-template-columns:34px minmax(0,1fr) auto;grid-template-areas:"favorite identity status" ". quota quota" ". source latency" ". actions actions";grid-template-rows:auto minmax(66px,auto) auto auto;align-items:start;gap:11px 10px;min-height:222px;padding:15px}
    .provider-list.cards .favorite{grid-area:favorite}
    .provider-list.cards .provider-main>.provider-cell:nth-of-type(1){grid-area:identity}
    .provider-list.cards .cell-status{grid-area:status;justify-self:end;text-align:right}
    .provider-list.cards .cell-quota{grid-area:quota;align-self:center;padding:8px 0 4px}
    ...
    .provider-list.cards .provider-message{min-height:32px;white-space:normal;overflow:visible;text-overflow:clip}
    ...
    .privacy{margin:14px 0 0;color:#78818b;font-size:11px;text-align:right}
    ...
    @media(max-width:1080px){.provider-columns,.provider-main{grid-template-columns:38px minmax(180px,1.25fr) 100px minmax(170px,1fr) minmax(145px,.9fr) minmax(214px,1fr)}.column-latency,.cell-latency{display:none}.provider-list.cards #provider-list{grid-template-columns:1fr}.provider-list.cards .cell-latency{display:block}}
    @media(max-width:840px){.shell{width:min(100% - 20px,1380px);padding-top:16px}.topbar{align-items:flex-start}.brand{display:block}.brand-mark{margin-bottom:4px}.toolbar{grid-template-columns:1fr auto}.tabs{grid-column:1/-1;overflow-x:auto}.result-line{align-items:flex-start;flex-direction:column;gap:2px}.result-line span{max-width:100%;overflow-wrap:anywhere}.task-group{grid-template-columns:minmax(130px,.55fr) minmax(180px,1.45fr)}.task-actions{grid-column:1/-1;justify-content:flex-start}.provider-columns{display:none}.provider-main{grid-template-columns:36px minmax(0,1fr);gap:8px 9px;min-height:86px}.provider-main>.favorite{grid-column:1;grid-row:1}.provider-main>.provider-cell:nth-of-type(1){grid-column:2;grid-row:1}.cell-status{grid-column:2;grid-row:2;justify-self:start}.cell-quota{grid-column:2;grid-row:3;padding-right:0}.cell-source{grid-column:2;grid-row:4;align-self:start}.row-actions{grid-column:2;grid-row:5;justify-content:flex-start;flex-direction:row}.provider-detail{padding-left:55px;flex-direction:column}.detail-actions{justify-content:flex-start}.provider-list.cards .provider-main{grid-template-columns:34px minmax(0,1fr) auto;grid-template-areas:"favorite identity status" ". quota quota" ". source latency" ". actions actions";grid-template-rows:auto minmax(66px,auto) auto auto;min-height:222px;padding:15px}.provider-list.cards .cell-status{grid-column:auto;grid-row:auto;justify-self:end}.provider-list.cards .cell-quota{grid-column:auto;grid-row:auto}.provider-list.cards .cell-source{grid-column:auto;grid-row:auto}.provider-list.cards .row-actions{grid-column:auto;grid-row:auto;justify-content:flex-end}.provider-list.cards .provider-detail{padding-left:15px}.overview{grid-template-columns:repeat(2,1fr)}.stat:nth-child(2){border-right:0}.stat:nth-child(-n+2){border-bottom:1px solid var(--line)}}
    @media(max-width:840px){.provider-list.cards .cell-status{grid-area:status}.provider-list.cards .cell-quota{grid-area:quota}.provider-list.cards .cell-source{grid-area:source}.provider-list.cards .cell-latency{grid-area:latency}.provider-list.cards .row-actions{grid-area:actions}}
    @media(max-width:560px){.topbar{display:block}.actions{justify-content:flex-start;margin-top:14px}.toolbar{grid-template-columns:1fr}.toolbar-controls{justify-content:space-between}.sort-select{width:100%;min-width:0}.overview{margin-top:14px}.task-center-head{align-items:flex-start;flex-direction:column;gap:2px}.task-group{grid-template-columns:1fr}.task-actions{grid-column:auto;justify-content:flex-start}.provider-main{padding-left:7px;padding-right:7px}.provider-detail{padding-left:49px}.provider-list.cards .provider-main{grid-template-columns:32px minmax(0,1fr);grid-template-areas:"favorite identity" ". status" ". quota" ". source" ". latency" ". actions";grid-template-rows:auto;min-height:0;padding:13px 11px}.provider-list.cards .cell-status{justify-self:start;text-align:left}.provider-list.cards .cell-latency{text-align:left}.provider-list.cards .row-actions{justify-content:flex-start}.provider-list.cards .provider-detail{padding:11px}.diagnostic-dialog{width:100vw;max-width:100vw;min-width:0;margin:0}.diagnostic-panel,.diagnostic-body{min-width:0}.diagnostic-summary{align-items:flex-start;flex-direction:column}.profile-row{grid-template-columns:minmax(0,1fr) auto}.profile-row>div:first-child{grid-column:1/-1}.alias-input{grid-column:1;min-width:0}.diagnostic-footer{flex-wrap:wrap}.method-fields,.template-grid{grid-template-columns:1fr}.method-row.wide{grid-column:auto}.dialog-head,.method-body{padding-left:14px;padding-right:14px}.dialog-head>div{min-width:0}.dialog-close{flex:0 0 auto}}
...
<body>
  <main class="shell">
    ...
    <section class="overview" aria-label="供应商汇总">
      <div class="stat"><span class="stat-label">全部供应商</span><strong id="count-all" class="stat-value">--</strong></div>
    ...
    <p class="privacy">本机只读 · Cookie 与认证原文不进入 Hub 页面</p>
  </main>
  ...
  <script nonce="${nonce}">
    ...
    let state={providers:[],refreshing:false,preferences:{version:2,favorites:[],sort:'smart',view:'cards',browserAliases:{},providerBrowsers:{}},diagnostics:{},browserOrigins:[]};
    let filter='all';
    let query='';
    let renderKey='';
    ...
    function providerNeedsAttention(item){return Boolean(item.accountBindingRequired||item.websiteLoginRequired||item.sessionSyncRequired||['error','login-required','degraded'].includes(String(item.status||'')))}
    ...
    function diagnosticCheckItems(){const providers=state.providers||[];const d=state.diagnostics||{};const companion=state.companion||{connected:false,clients:[]};const browserCount=providers.filter(providerNeedsBrowser).length;const attentionCount=providers.filter(providerNeedsAttention).length;const versions=(companion.clients||[]).map(client=>String(client.version||'')).filter(Boolean);const expected=String(d.expectedCompanionVersion||'');const versionOk=!versions.length||!expected||versions.every(value=>value===expected);return[
      ...
      {tone:providers.length>0&&d.databaseWatch?'ok':providers.length>0?'warning':'error',title:'CCSwitch 数据源',detail:providers.length+' 个 Codex 供应商'+(d.databaseWatch?' · 变更监听正常':' · 变更监听未就绪')},
    ...
    refreshAll.addEventListener('click',async()=>{try{await post('refresh');showToast('已开始刷新全部供应商');scheduleOperationMonitor()}catch(error){showToast(error.message)}await load()});
    ...
    load().then(()=>{scheduleCompanionMonitor();maybeAutoCheckPending();if(location.hash==='#diagnostics'||new URLSearchParams(location.search).get('view')==='diagnostics')openDiagnostics()});
  </script>
</body>
</html>`;
}
