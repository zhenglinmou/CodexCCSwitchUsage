...
<body>
  ...
  <script nonce="${nonce}">
    ...
    const sourceNames={official_api:'官方 API',provider_api:'供应商 API',api_key_probe:'API Key 能力检查',jianzhile_api_key:'简直了 API Key 额度',jianzhile_account:'简直了账户总额度',freely_api_key:'freely API Key 额度',freely_account:'freely 账户总额度',muyuan_api_key:'君的公益 API Key 额度',muyuan_account:'君的公益账户总额度',muyuan_browser:'君的公益浏览器查询',openai_wham:'OpenAI 用量接口',openai_wham_browser:'OpenAI 用量接口（现有浏览器）',cpa_auth_files:'CLIProxyAPI 本地账号',browser_session:'现有浏览器会话',cached_previous:'上次缓存',api_health_and_local_usage:'API 健康检查 + 本地统计'};
