...
<body>
  ...
  <script nonce="${nonce}">
    ...
    function stateRenderKey(next){const companion=next.companion||{connected:false,clients:[]};const clients=(companion.clients||[]).map(client=>[client.ref,client.browser,client.version,[...(client.sessions||[])].sort()]);return JSON.stringify([next.revision,next.refreshing,next.lastFullRefreshAt,companion.connected,clients])}
    ...
    document.addEventListener('visibilitychange',()=>{if(document.visibilityState==='visible')load().then(()=>{if(state.refreshing)scheduleOperationMonitor()})});
    load();
  </script>
</body>
</html>`;
